...
- #faces
- coor des vertices
- descr des faces (de 0 à v - 1)
    - #vertices ds la face
    - description de la face par les indices de ces vertices
      (sens trigo)

Ds notre mesh, y a :

- TabVertices : chaque sommet c (x; y; z) & l'indice d'une face
- TabFaces : chaque face c les indices des 3 sommets, et des 3 faces adjacentes

On a une map `std::map<std::pair, int>` où une clef c une arête et une valeur c un indice de face.
Qd une nouvelle face spawn, on regarde si une arête est déjà connue de notre map, auquel cas la nouvelle face a déjà une voisine connue.
NB: en valeur on peut en + avoir l'indice local de l'arête

La recherche ds une map (c un arbre rouge-noir en C++) c en $O(\log n)$ (effectivement pr une hashmap ça serait du $O(1)$).
